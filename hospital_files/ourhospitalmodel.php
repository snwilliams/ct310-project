...
//	return $to_return;
    ...
    public static function get_hospital_details($query){
        return DB::query(
            "SELECT DISTINCT provider_name, provider_street_address, provider_city, provider_state,
            hospital_referral_region, DRG_Number, DRG_Description, average_covered_charges, average_total_payments, average_medicare_payments FROM `medicare` WHERE provider_id = $query",
            DB::SELECT)->execute()->as_array();

    }

    public static function get_hospital_details_default(){
        $provider_query = $_GET['num'];
        return DB::query("SELECT DISTINCT provider_name, provider_street_address, provider_city, provider_state,
        hospital_referral_region, DRG_Number, DRG_Description, average_covered_charges, average_total_payments, average_medicare_payments FROM `medicare`
        WHERE provider_id = $provider_query", DB::SELECT)->execute()->as_array();
